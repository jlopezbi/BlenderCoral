...
class BoxWorld(object):
    ...
    def __init__(self, front_vertex, back_vertex):
        ...
        greater_or_equal = (
            np.greater(self.upper_vertex, self.lower_vertex).all()
            or np.equal(self.upper_vertex, self.lower_vertex).all()
        )
        assert greater_or_equal, "front vertex {} must be greater than back_vertex {}".format(
    ...
    def show(self):
        ...
        viz.add_box(self.lower_vertex, self.upper_vertex)
    ...
    def resize_to_fit(self, bbox_lower, bbox_upper, padding=0.0):
        ...
        """
        lower_offset = np.array((padding, padding, 0.0))
